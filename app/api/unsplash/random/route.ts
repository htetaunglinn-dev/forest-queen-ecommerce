/**
 * Server-Side Unsplash Random Image API Route
 *
 * This route handles random image requests server-side,
...
// Cache TTL: 1 hour (random images can be cached shorter)
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cacheKey = `unsplash:random:${query}:${count}:${orientation}`;
    ...
    const data = await withCache(
      ...
      async () => {
        ...
        console.log(`[API] Fetching random from Unsplash: ${query || 'no query'} (count: ${count})`);
    ...
    console.error('[API] Unsplash random error:', error);

    return NextResponse.json(
      {
        error: 'Failed to fetch random images from Unsplash',
